...
| `resume { token }`                    | Introduced; not in a room; not logged in; token issued by a login or registration and not revoked; account not held by another live session.     | `ALREADY_IN_ROOM`, `ALREADY_LOGGED_IN`, `SERVER_ERROR` (an unknown token replies `accountState { account: null }`, no error) |
| `logout`                              | Introduced; not in a room. Revokes the token this session used.                                                                                  | `ALREADY_IN_ROOM` (replies `accountState { account: null }` otherwise)                                                       |
...
| `deleteMap { id }`                    | Introduced; `id` names a stored (not built-in) map.                                                                                              | `MAP_READONLY`, `MAP_NOT_FOUND`, `SERVER_ERROR`                                                                              |
...
A login or a registration also hands out a **session token** (32 random bytes, hex) in
`accountState.token`; the account record keeps only its SHA-256 (`sessionTokens`, the five most
recent, oldest dropped) so the file never holds a working token. The client stores the token in
`localStorage` (`SessionStore`) and sends `resume { token }` right after `hello` on every
connection, holding any other action until the reply, so a reloaded page comes back logged in
under the right name. A disconnection frees the account but keeps the token; `logout` revokes the
token the session used and the client forgets it, as it does whenever the server answers
`accountState { account: null }` (the reply to an unknown or revoked token, sent without an error).
