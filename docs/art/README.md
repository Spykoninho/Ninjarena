...
**Revue.** La page art-review ajoute une vue **Poses** (une grille de ninjas qui rejouent en boucle attaque au kunai, lancer de shuriken, cast, hit, dash et mort dans les quatre directions, plus un shuriken et une boule de feu en vol) et, sous la scène, une planche statique de toutes les poses générées : c’est la vérité des textures, indépendante du temps. Le HUD y tourne sur des données factices. Aucune de ces vues n’est un mode de jeu.

## Capacités 1.8

Le roster passe de dix à vingt-trois capacités, et chacune reçoit son illustration. Tout reste dessiné par les recettes natives, sans atlas.

**Illustrations.** `abilityIcons.ts` porte un dessin 24 × 24 par identifiant de capacité (kunaï, shuriken, bâton, poing, aiguilles, éclair, bouclier, sceau, flamme, sol fissuré, mur, mine, tourbillon, bélier, kunaï cloué, éventail, cône de givre, colonne céleste, fumée, traits de vent, méditation), affiché ×2 dans le HUD, sur les cartes du salon et dans le kit du roster. Une capacité sans dessin retombe sur l’icône de sa famille ; trois familles s’ajoutent pour cela (`support`, `stealth`, `haste`). La page art-review termine par la planche de toutes les icônes à l’échelle du HUD.

**Télégraphes.** Une mêlée annonce désormais le cône exact qu’elle frappera (portée et ouverture du gameplay, bord qui se remplit avec l’incantation, plein cercle pour un tourbillon), à la place de l’orbe générique. La Frappe céleste introduit la marque `sky-mark` de la bible : périmètre exact, croix à centre vide et quatre traits qui convergent vers la marque pendant l’attente ; au déclenchement, une colonne fine tombe sur la marque puis se resserre et s’éteint en 320 ms, sans jamais couvrir la zone pendant l’anticipation. Un renfort (soin, voile, foulée) se signale par des tirets qui se resserrent sur le lanceur.

**Formes en vol.** Le `style` d’un `visual` nomme la forme dessinée : `needle` (trait fin, pointe sombre), `kunai` (lame, poignée de bois, anneau et fil qui traîne), `shuriken` (étoile tournante), `column` (zone dont la marque garde la croix céleste). Un `melee` porte une `color` : le croissant de coupe et l’éventail prennent la teinte de la technique (givre, bois du bâton, or du poing).

**Accessoires.** L’attaque de base en cours choisit l’objet tenu : kunaï, shuriken, bâton qui pivote autour de la main sur les quatre temps (traînée cyan à l’extension), poing gainé avec éclat or à l’impact, ou trois aiguilles entre les doigts avant le lancer.

**Statuts.** `HASTED` dessine des traits de vitesse derrière le corps, du côté opposé à la visée ; `INVISIBLE` laisse le porteur et ses alliés voir le voilé en transparence avec quelques volutes, pendant que les autres équipes ne le dessinent pas. Une bouffée de fumée (trois disques qui s’écartent et pâlissent) part à la disparition, une gerbe menthe à l’accélération, et un soin monte en chiffre menthe précédé d’un `+`.

## Équilibrage 1.9

Les attaques à distance gagnent un peu d’incantation (40 à 70 ms) et un peu de portée (environ 20 %) ; tous les étourdissements doublent. La Méditation coûte la moitié de la réserve de chakra jusqu’à la fin de la manche. La Frappe céleste tombe sous le curseur (le client envoie la distance du curseur avec la visée), la marque au sol suit. La Frappe sismique devient une **vague** : un projectile perforant, lent et large, dessiné comme un croissant de sol soulevé avec des éclats qui roulent devant, qui traverse chaque cible une fois et s’arrête au premier mur ; son télégraphe est le couloir fléché. Les Mines explosives sèment sept petits engins autour du lanceur : un disque d’encre à pointes, voyant qui clignote de plus en plus vite, souffle indiqué par un contour discret ; une mine touchée par un tir, un coup ou une explosion adverse saute, et toute la grappe avec elle.
